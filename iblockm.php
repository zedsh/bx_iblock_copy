    ...
    foreach($PROPERTIES as $prop_fields){
        ...
        if($prop_fields['PROPERTY_TYPE'] == 'E'){
            if(!isset($MAP_IBLOCKS[$prop_fields["LINK_IBLOCK_ID"]])){
                to_log('NOT SET MAP FROM '.$prop_fields["LINK_IBLOCK_ID"]);
                //ignore unlinked
            }
...
function copy_iblock($all_iblocks,$IBLOCK_ID,$REPLACE,$MAP_IBLOCKS,&$MAP_ELEMENTS)
{
    ...
    foreach($user_types['IBLOCK'] as $utype){
        if($utype['USER_TYPE_ID'] === 'file'){
            $file_fields[] = $utype['FIELD_NAME'];
        }
    ...
    foreach($user_types['SECTION'] as $utype){
        if($utype['USER_TYPE_ID'] === 'file'){
            $section_file_fields[] = $utype['FIELD_NAME'];
        }
    ...
    foreach($sections as $id => $section){
        ...
        foreach($section_file_fields as $section_file_field){
            if(!empty($section[$section_file_field])){
                $section[$section_file_field] = process_file_field($section[$section_file_field]);
            }
        }

        if(!empty($section_file_fields)){
            to_log('COPY UF!');
            to_log($section);
            to_log('END COPY UF!');
        }
    ...
    foreach($elements as $element){
        ...
        foreach($file_fields as $file_field){
            if(!empty($element[$file_field])){
                $element[$file_field] = process_file_field($element[$file_field]);
            }
        }
...
//    // exit;
    ...
    foreach($iblocks as $iblock){
         $REPLACE = array('IBLOCK_TYPE_ID' => 'ua','LID' => 'ua');

         $ret = copy_iblock($all_iblocks,$iblock,$REPLACE,$new_iblocks,$map_elements);
